...
valid_plugin_filename_strategy = st.from_regex(r"^[a-z][a-z0-9_]{0,15}\.py$", fullmatch=True)
...
class TestCollectionStructureProperties:
    ...
    def test_collection_with_random_structure_always_constructs(
        ...
        plugins = {PluginType.MODULE: [f"module_{i}.py" for i in range(num_modules)]}
    ...
    def test_list_plugins_by_type_returns_correct_count(
        ...
        """
        plugins = {plugin_type: [f"plugin_{i}.py" for i in range(plugin_count)]}
    ...
    def test_large_collections_perform_reasonably(
        ...
        """
        roles = [f"role_{i}" for i in range(role_count)]
        plugins = {PluginType.MODULE: [f"module_{i}.py" for i in range(module_count)]}
