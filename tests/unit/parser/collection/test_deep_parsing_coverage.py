...
class TestPluginParserCoverage:
    def test_parse_non_existent_file(self):
        ...
        parser = PluginParser()
        plugin = Plugin(
            name="test_plugin", type=PluginType.MODULE, path=Path("/non/existent/path.py")
        )
    ...
    def test_parse_syntax_error(self, tmp_path):
        ...
        parser = PluginParser()
        plugin = Plugin(name="syntax_error", type=PluginType.MODULE, path=plugin_path)
...
'''
        ...
        parser = PluginParser()
        plugin = Plugin(name="invalid_yaml", type=PluginType.MODULE, path=plugin_path)
    ...
    def test_parse_legacy_string_node(self, tmp_path):
        ...
        with (
            patch("ansibledoctor.parser.plugin_parser.ast.Constant", type("DummyConstant", (), {})),
