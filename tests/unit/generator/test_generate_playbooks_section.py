...
class TestGeneratePlaybooksSection:
    ...
    def mock_collection(self):
        metadata = GalaxyMetadata(
            namespace="test_ns", name="test_coll", version="1.0.0", authors=["me"], dependencies={}
        )
        playbooks = [
            PlaybookInfo(
                name="site",
            ...
            PlaybookInfo(
                name="db", path="/path/to/db.yml", description="Database playbook", tags=["db"]
            ),
        ]
        return AnsibleCollection(metadata=metadata, roles=[], plugins={}, playbooks=playbooks)
