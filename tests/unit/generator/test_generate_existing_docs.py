...
def mock_collection_with_docs():
    metadata = GalaxyMetadata(
        namespace="test_ns", name="test_coll", version="1.0.0", authors=["me"], license=["MIT"]
    )
    ...
    return AnsibleCollection(metadata=metadata, existing_docs=existing_docs)
...
def test_context_includes_existing_docs(mock_collection_with_docs):
    ...
    context_builder = CollectionTemplateContext(collection=mock_collection_with_docs, plugins=[])
    context = context_builder.build()

    assert "collection" in context
    assert context["collection"].existing_docs is not None
    assert (
        context["collection"].existing_docs.readme_content == "# My README\n\nThis is the readme."
    )
