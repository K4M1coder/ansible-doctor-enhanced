...
class TestCollectionGenerateCLI:
    ...
    def test_generate_collection_docs_markdown(self, minimal_collection_path, tmp_path):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "ansibledoctor", "collection", "generate", 
             str(minimal_collection_path), "--output-dir", str(output_dir)],
            capture_output=True,
            text=True,
            check=False
        )
    ...
    def test_generate_collection_docs_html(self, minimal_collection_path, tmp_path):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "ansibledoctor", "collection", "generate",
             str(minimal_collection_path), "--output-dir", str(output_dir), "--format", "html"],
            capture_output=True,
            text=True,
            check=False
        )
    ...
    def test_generate_collection_docs_rst(self, minimal_collection_path, tmp_path):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "ansibledoctor", "collection", "generate",
             str(minimal_collection_path), "--output-dir", str(output_dir), "--format", "rst"],
            capture_output=True,
            text=True,
            check=False
        )
    ...
    def test_generate_realistic_collection_docs(self, realistic_collection_path, tmp_path):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "ansibledoctor", "collection", "generate",
             str(realistic_collection_path), "--output-dir", str(output_dir)],
            capture_output=True,
            text=True,
            check=False
        )
    ...
    def test_generate_nonexistent_collection(self, tmp_path):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "ansibledoctor", "collection", "generate",
             str(nonexistent_path), "--output-dir", str(output_dir)],
            capture_output=True,
            text=True,
            check=False
        )
...
class TestCollectionCLIWorkflow:
    ...
    def test_parse_then_generate_workflow(self, realistic_collection_path, tmp_path):
        ...
        parse_result = subprocess.run(
            [sys.executable, "-m", "ansibledoctor", "collection", "parse",
             str(realistic_collection_path), "--output", str(parse_output), "--pretty"],
            capture_output=True,
            text=True,
            check=False
        )
        ...
        generate_result = subprocess.run(
            [sys.executable, "-m", "ansibledoctor", "collection", "generate",
             str(realistic_collection_path), "--output-dir", str(docs_dir)],
            capture_output=True,
            text=True,
            check=False
        )
...
class TestCollectionAnalyzeCLI:
    ...
    def test_analyze_collection_check_circular(self, realistic_collection_path):
        ...
        result = subprocess.run(
            [
                sys.executable, "-m", "ansibledoctor", 
                "collection", "analyze", 
