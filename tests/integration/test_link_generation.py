...
pytestmark = pytest.mark.skip(
    reason="TDD Red phase stubs - awaiting implementation with proper parser APIs"
)
...
class TestBrowserNavigation:
    ...
    def test_links_support_browser_back_button(self, tmp_path):
        ...
        link_to_b = link_manager.create_link(
            source=role_a / "README.md", target=role_b / "README.md", text="Go to AnsibleRole B"
        )
...
"""
        ...
        anchor_link = link_manager.create_link(
            source=readme, target=readme.as_posix() + "#section-2", text="Go to Section 2"
        )
...
"""
        ...
        for target in targets:
            ...
            assert (
                f"## {anchor.replace('-', ' ')}" in content_lower
                or f"# {anchor.replace('-', ' ')}" in content_lower
            )
...
"""
        ...
        assert (
            ("<details>" in toc and "<summary>" in toc)
            or ("collapse" in toc.lower())
            or ("mobile" in toc.lower())
            or "<nav" in toc
        )
...
"""
        ...
        assert (
            "http://" not in toc
            and "https://" not in toc
            or toc.count("http") == 0
            or "cdn" not in toc.lower()
        )
