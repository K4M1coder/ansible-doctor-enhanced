...
class TestBestPracticesLinks:
    ...
    def test_security_keyword_links_to_security_guide(self, tmp_path: Path) -> None:
        ...
        assert any(
            "security" in link.target.lower() or "vault" in link.target.lower() for link in links
        )
    ...
    def test_testing_keyword_links_to_testing_guide(self, tmp_path: Path) -> None:
        ...
        assert any(
            "test" in link.target.lower() or "molecule" in link.target.lower() for link in links
        )
