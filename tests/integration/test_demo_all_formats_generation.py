...
def test_generate_all_formats_for_demo_role(tmp_path):
    ...
    demo_role = repo_root / "demo" / "role_demo_namespace.demo_demo_role"
    ...
    for fmt in FORMATS:
        ...
        result = runner.invoke(cli, ["generate", str(demo_role), "--format", fmt, "--output", str(out_file)])
...
def test_generate_all_formats_for_demo_collection(tmp_path):
    ...
    for fmt in FORMATS:
        ...
        result = runner.invoke(cli, ["collection", "generate", str(demo_collection), "--format", fmt])
...
def test_generate_all_formats_for_demo_project(tmp_path):
    ...
    for fmt in FORMATS:
        ext = "md" if fmt == "markdown" else ("html" if fmt == "html" else "rst")
        result = runner.invoke(cli, ["project", "generate", str(demo_project), "--format", fmt])
