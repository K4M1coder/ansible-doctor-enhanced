...
class RoleMetadata(BaseModel):
    ...
    galaxy_tags: list[str] = Field(
        default_factory=list, description="Galaxy search tags"
    )
    
    # Dependencies
    dependencies: list[Dependency] = Field(
        default_factory=list, description="Role dependencies"
    )
    ...
    meta_file_path: Optional[str] = Field(
        None, description="Path to meta/main.yml for context"
    )
    ...
    def get_supported_platforms_summary(self) -> list[str]:
        ...
        """
        return [
            f"{p.name} ({', '.join(p.versions)})" if p.versions else p.name
