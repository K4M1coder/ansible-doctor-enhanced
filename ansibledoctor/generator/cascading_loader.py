...
class CascadingTemplateLoader:
    ...
    def find_template(
        ...
        if cache_key in self._cache:
            ...
            if (datetime.now() - cached_at).total_seconds() < self._cache_ttl:
                log.debug(
                    "Template cache hit",
                    extra={"template": name, "cache_key": cache_key}
                )
    ...
    def _build_search_order(
        ...
        if (resolved_path.parent.name == "roles" and
            (resolved_path.parent.parent / "galaxy.yml").exists()):
            coll_templates = (
                resolved_path.parent.parent / ".ansibledoctor" / "templates"
            )
