...
class TodoParser:
    ...
    SKIP_DIRS = {'.git', '.hypothesis', '__pycache__', 'htmlcov', '.pytest_cache', 
        ...
        while i < len(lines):
            ...
            if match:
                ...
                while i < len(lines):
                    ...
                    if (cont_match and 
                        lines[i].lstrip() != lines[i] and  # Has indentation
                        not lines[i].strip().startswith(('---', 'var:', '-', '@'))):
                        # Continuation line
    ...
    def parse_role(self, role_path: Path) -> list[TodoItem]:
        ...
        for todo in todos:
            try:
                abs_path = Path(todo.file_path) if not Path(todo.file_path).is_absolute() else Path(todo.file_path)
